...
def nn_forward_backward(
    ...
    data_loss = -log_probabilities[rows, y].mean()
    regularization_loss = reg * ((W1 * W1).sum() + (W2 * W2).sum())
    ...
    grads["W2"] = h1.t().mm(score_gradient) + 2 * reg * W2
    grads["b2"] = score_gradient.sum(dim=0)

    hidden_gradient = score_gradient.mm(W2.t())
    hidden_gradient[h1 <= 0] = 0
    grads["W1"] = X.t().mm(hidden_gradient) + 2 * reg * W1
    grads["b1"] = hidden_gradient.sum(dim=0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_train(
    ...
    for it in range(num_iters):
        ...
        for parameter_name in params:
            params[parameter_name] -= learning_rate * grads[parameter_name]
        #########################################################################
        #                             END OF YOUR CODE                          #
        #########################################################################
        ...
        for hidden_size in hidden_sizes:
            for reg in regularization_strengths:
                for learning_rate_decay in learning_rate_decays:
                    ...
                    val_acc = (val_predictions == y_val).double().mean().item()
