...
def svm_loss_vectorized(
    ...
    num_train = X.shape[0]
    scores = X.mm(W)
    correct_scores = scores[torch.arange(num_train, device=X.device), y]
    margins = (scores - correct_scores.reshape(-1, 1) + 1).clamp(min=0)
    ...
    #############################################################################
    # Replace "pass" statement with your code
    positive = (margins > 0).to(W.dtype)
    positive_counts = positive.sum(dim=1)
    positive[torch.arange(num_train, device=X.device), y] = -positive_counts
    dW = X.t().mm(positive) / num_train + 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def test_one_param_set(
    ...
    train_acc = (train_pred == data_dict["y_train"]).double().mean().item()
    val_acc = (val_pred == data_dict["y_val"]).double().mean().item()
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        ...
        loss += -shifted_scores[y[i]] + normalizer.log()

        probabilities = exp_scores / normalizer
        for j in range(num_classes):
    ...
    loss = loss / num_train + reg * (W * W).sum()
...
def softmax_loss_vectorized(
    ...
    log_probabilities = shifted_scores - exp_scores.sum(
        dim=1, keepdim=True
    ).log()
    loss = -log_probabilities[rows, y].mean() + reg * (W * W).sum()

    score_gradient = probabilities.clone()
    score_gradient[rows, y] -= 1
    score_gradient /= num_train
    dW = X.t().mm(score_gradient) + 2 * reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
