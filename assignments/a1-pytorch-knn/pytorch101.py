...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    first = (start + 9) // 10
    last = stop // 10
    x = 10 * torch.arange(first, last + 1, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    row_indices = torch.arange(x.shape[0] - 1, -1, -1, device=x.device)
    y = x[row_indices]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    y = torch.zeros(
        (x_tensor.shape[0], x_tensor.max().item() + 1),
        dtype=torch.float32,
    )
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    y[torch.arange(x.shape[0], device=x.device), min_indices] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    for batch_index in range(x.shape[0]):
        z[batch_index] = x[batch_index].mm(y[batch_index])
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return z
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    centered = x - column_means.reshape(1, -1)
    column_variances = (centered ** 2).sum(dim=0) / (x.shape[0] - 1)
    y = centered / (column_variances ** 0.5).reshape(1, -1)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    lengths = ls.to(device=xs[0].device)
    cumulative = torch.cat(xs).cumsum(dim=0)
    cumulative_at_ends = cumulative[lengths.cumsum(dim=0) - 1]
    previous = torch.cat(
        (cumulative_at_ends.new_zeros(1), cumulative_at_ends[:-1])
    )
    y = (cumulative_at_ends - previous) / lengths
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    boundary_indices = torch.nonzero(
        sorted_x[1:] != sorted_x[:-1], as_tuple=False
    ).flatten() + 1
    boundary_indices = torch.cat((x[:1] * 0, boundary_indices))
    uniques = sorted_x[boundary_indices]
    indices = sorted_indices[boundary_indices]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
