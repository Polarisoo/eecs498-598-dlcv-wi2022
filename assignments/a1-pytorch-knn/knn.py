...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        train_row = x_train[i].reshape(-1)
        for j in range(num_test):
            difference = train_row - x_test[j].reshape(-1)
            dists[i, j] = (difference * difference).sum()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        difference = x_train[i].reshape(1, -1) - x_test_flat
        dists[i] = (difference * difference).sum(dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    dists.mul_(-2)
    dists.add_((x_train_flat * x_train_flat).sum(dim=1).reshape(-1, 1))
    dists.add_((x_test_flat * x_test_flat).sum(dim=1).reshape(1, -1))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    nearest_indices = torch.topk(
        dists, k=k, dim=0, largest=False
    ).indices
    y_pred = torch.zeros(
        num_test, dtype=torch.int64, device=y_train.device
    )
    for j in range(num_test):
        nearest_labels = y_train[nearest_indices[:, j]]
        y_pred[j] = torch.bincount(nearest_labels).argmax()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
    ...
    for k in k_choices:
        ...
        for fold in range(num_folds):
            ...
            x_training = torch.cat(
                x_train_folds[:fold] + x_train_folds[fold + 1:], dim=0
            )
            y_training = torch.cat(
                y_train_folds[:fold] + y_train_folds[fold + 1:], dim=0
            )
            classifier = KnnClassifier(x_training, y_training)
            accuracy = classifier.check_accuracy(
                x_validation, y_validation, k=k, quiet=True
            )
