...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, layer in enumerate(style_layers):
        ...
        loss = loss + style_weights[i] * (gram - style_targets[i]).pow(2).sum()
    return loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def tv_loss(img, tv_weight):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    h_var = (img[:, :, 1:, :] - img[:, :, :-1, :]).pow(2).sum()
    w_var = (img[:, :, :, 1:] - img[:, :, :, :-1]).pow(2).sum()
...
def guided_gram_matrix(features, masks, normalize=True):
  ...
  masked = features * masks.unsqueeze(2)
  feats = masked.view(N, R, C, H * W)
  guided_gram = torch.matmul(feats, feats.transpose(2, 3))
  if normalize:
...
def guided_style_loss(feats, style_layers, style_targets, style_weights, content_masks):
    ...
    loss = 0
    for i, layer in enumerate(style_layers):
        guided_gram = guided_gram_matrix(feats[layer], content_masks[layer])
        loss = loss + style_weights[i] * (
            guided_gram - style_targets[i]
        ).pow(2).sum()
    return loss
    ############################################################################
    #                               END OF YOUR CODE                           #
