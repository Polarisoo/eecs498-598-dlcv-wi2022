...
def compute_saliency_maps(X, y, model):
    ...
    scores = model(X)
    correct_scores = scores.gather(1, y.view(-1, 1)).squeeze(1)
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for it in range(max_iter):
        ...
        if pred_y.item() == target_y:
            if verbose:
                print(
                    "Fooled the model after %d iterations; target score: %.3f"
                    % (it, scores[0, target_y].item())
                )
            break
        ...
        if verbose:
            print(
                ...
                % (it, target_score.item(), scores[0, pred_y].item())
            )
