...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        step_dx, dprev_h, step_dWx, step_dWh, step_db = rnn_step_backward(
            dh[:, t, :] + dprev_h, step_caches[t]
        )
        dx[:, t, :] = step_dx
        dWx += step_dWx
        dWh += step_dWh
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    loss = F.cross_entropy(x.permute(0, 2, 1), y, ignore_index=-100 if ignore_index is None else ignore_index, reduction="sum") / x.shape[0]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        if cell_type == "rnn":
            ...
            self.feature_projection = nn.Conv2d(input_dim, hidden_dim, kernel_size=1)
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == "attn":
            h0_or_A = self.feature_projection(features)
        else:
            pooled_features = features.mean(dim=(2, 3))
            h0_or_A = self.feature_projection(pooled_features)

        word_vectors = self.word_embedding(captions_in)
        hidden_states = self.rnn(word_vectors, h0_or_A)
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == "attn":
            ...
            pooled_features = features.mean(dim=(2, 3))
            prev_h = self.feature_projection(pooled_features)
            prev_c = torch.zeros_like(prev_h)

        prev_word = images.new_full((N,), self._start, dtype=torch.long)
        for t in range(max_length):
            word_vec = self.word_embedding(prev_word[:, None])[:, 0, :]
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    A_flat = A.reshape(N, H, D_a * D_a).permute(0, 2, 1)
    scores = torch.bmm(A_flat, prev_h.unsqueeze(2)).squeeze(2) / math.sqrt(H)
