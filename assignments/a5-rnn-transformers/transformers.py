...
def scaled_dot_product_two_loop_single(
    ...
    # Replace "pass" statement with your code
    K, M = query.shape
    out = value.new_zeros(K, M)
    for i in range(K):
        weights = value.new_zeros(K)
        for j in range(K):
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    out = value.new_zeros(N, K, M)
    for n in range(N):
        weights = value.new_zeros(K, K)
        for i in range(K):
            weights[i] = torch.mv(key[n], query[n, i]) / (M ** 0.5)
        weights = F.softmax(weights, dim=1)
        out[n] = weights.mm(value[n])
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        bound = (6 / (self.linear.in_features + self.linear.out_features)) ** 0.5
    ...
    def forward(
        ...
        y = self.linear(torch.cat(head_outputs, dim=2))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \\ 
        ...
        self.attention = self.MultiHeadBlock
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    y = positions.view(1, K, 1).repeat(1, 1, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
