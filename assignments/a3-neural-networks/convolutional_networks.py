...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        out = torch.empty(N, F, out_height, out_width, dtype=x.dtype, device=x.device)
        for n in range(N):
            for f in range(F):
                for i in range(out_height):
                    ...
                    for j in range(out_width):
                        ...
                        window = padded_x[n, :, height_start:height_start + HH,
                                          width_start:width_start + WW]
            ...
            for f in range(F):
                for i in range(out_height):
                    ...
                    for j in range(out_width):
                        ...
                        grad = dout[n, f, i, j]
                        dw[f] += window * grad
                        dx_padded[n, :, height_start:height_start + HH,
                                  width_start:width_start + WW] += w[f] * grad
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        out = torch.empty(N, C, out_height, out_width, dtype=x.dtype, device=x.device)
        for i in range(out_height):
            height_start = i * stride
            for j in range(out_width):
                width_start = j * stride
                window = x[:, :, height_start:height_start + pool_height,
    ...
    def backward(dout, cache):
        ...
        for i in range(out_height):
            ...
            for j in range(out_width):
                ...
                window = x[:, :, height_start:height_start + pool_height,
                           width_start:width_start + pool_width]
                max_values = window.amax(dim=(2, 3), keepdim=True)
                mask = (window == max_values).to(x.dtype)
                dx[:, :, height_start:height_start + pool_height,
                   width_start:width_start + pool_width] += (
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        # Replace "pass" statement with your code
        C, H, W = input_dims
        pooled_height, pooled_width = H // 2, W // 2
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        pooled, conv_cache = Conv_ReLU_Pool.forward(
            X, W1, b1, conv_param, pool_param)
        hidden, hidden_cache = Linear_ReLU.forward(pooled, W2, b2)
        scores, output_cache = Linear.forward(hidden, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dhidden, dW3, db3 = Linear.backward(dscores, output_cache)
        dpooled, dW2, db2 = Linear_ReLU.backward(dhidden, hidden_cache)
        _, dW1, db1 = Conv_ReLU_Pool.backward(dpooled, conv_cache)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        C, H, W = input_dims
        previous_channels = C
        for i, filters in enumerate(num_filters, start=1):
            if weight_initializer is not None:
                weight = weight_initializer(previous_channels, filters, K=3,
                                            relu=True, device=device, dtype=dtype)
            elif weight_scale == 'kaiming':
                weight = kaiming_initializer(previous_channels, filters, K=3,
                                             relu=True, device=device, dtype=dtype)
            else:
                weight = weight_scale * torch.randn(
                    filters, previous_channels, 3, 3, dtype=dtype, device=device)
            self.params['W%d' % i] = weight
            self.params['b%d' % i] = torch.zeros(filters, dtype=dtype, device=device)
            if self.batchnorm:
                self.params['gamma%d' % i] = torch.ones(filters, dtype=dtype, device=device)
                self.params['beta%d' % i] = torch.zeros(filters, dtype=dtype, device=device)
            if i - 1 in self.max_pools:
                H //= 2
                W //= 2
            previous_channels = filters

        final_input_dim = previous_channels * H * W
        if weight_initializer is not None:
            final_weight = weight_initializer(final_input_dim, num_classes, K=None,
                                              relu=False, device=device, dtype=dtype)
        elif weight_scale == 'kaiming':
            final_weight = kaiming_initializer(final_input_dim, num_classes, K=None,
                                               relu=False, device=device, dtype=dtype)
        else:
            final_weight = weight_scale * torch.randn(
                final_input_dim, num_classes, dtype=dtype, device=device)
        self.params['W%d' % self.num_layers] = final_weight
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            if self.batchnorm:
                if layer_index in self.max_pools:
                    dout, dW, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dout, caches[layer_index])
                else:
                    dout, dW, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dout, caches[layer_index])
                grads['gamma%d' % i] = dgamma
                grads['beta%d' % i] = dbeta
            else:
                if layer_index in self.max_pools:
                    dout, dW, db = Conv_ReLU_Pool.backward(dout, caches[layer_index])
                else:
                    dout, dW, db = Conv_ReLU.backward(dout, caches[layer_index])
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            sample_var = centered.square().mean(dim=0)
            inv_std = torch.rsqrt(sample_var + eps)
            x_hat = centered * inv_std
            out = gamma * x_hat + beta
            cache = ('train', x_hat, gamma, centered, inv_std)
            running_mean = momentum * running_mean + (1 - momentum) * sample_mean
            running_var = momentum * running_var + (1 - momentum) * sample_var
            ################################################################
            #                           END OF YOUR CODE                   #
            ...
            out = gamma * x_hat + beta
            cache = ('test', x_hat, gamma, x - running_mean, inv_std)
            ################################################################
            #                      END OF YOUR CODE                        #
            ################################################################
        else:
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        mode, x_hat, gamma, centered, inv_std = cache
        dbeta = dout.sum(dim=0)
        dgamma = (dout * x_hat).sum(dim=0)
        if mode == 'train':
            N = dout.shape[0]
            dx_hat = dout * gamma
            dvar = (dx_hat * centered * (-0.5) * inv_std.pow(3)).sum(dim=0)
            dmean = (-dx_hat * inv_std).sum(dim=0)
            dmean += dvar * (-2.0 * centered).sum(dim=0) / N
            dx = dx_hat * inv_std + dvar * 2.0 * centered / N + dmean / N
        else:
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        flat_out, bn_cache = BatchNorm.forward(flat_x, gamma, beta, bn_param)
        out = flat_out.reshape(N, H, W, C).permute(0, 3, 1, 2)
    ...
    def backward(dout, cache):
        ...
        #################################################################
        # Replace "pass" statement with your code
        bn_cache, original_shape = cache
        N, C, H, W = original_shape
        flat_dout = dout.permute(0, 2, 3, 1).reshape(-1, C)
        flat_dx, dgamma, dbeta = BatchNorm.backward_alt(flat_dout, bn_cache)
        dx = flat_dx.reshape(N, H, W, C).permute(0, 3, 1, 2)
        ##################################################################
        #                       END OF YOUR CODE                         #
        ##################################################################
