...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        _, dW1, db1 = Linear_ReLU.backward(dhidden, hidden_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers + 1):
            self.params['W%d' % i] = weight_scale * torch.randn(
                layer_dims[i - 1], layer_dims[i], dtype=dtype, device=device)
            self.params['b%d' % i] = torch.zeros(layer_dims[i], dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        dout, dW, db = Linear.backward(dout, output_cache)
        grads['W%d' % self.num_layers] = dW + self.reg * self.params['W%d' % self.num_layers]
        grads['b%d' % self.num_layers] = db
        for i in range(self.num_layers - 1, 0, -1):
            if self.use_dropout:
                dout = Dropout.backward(dout, dropout_caches[i - 1])
            dout, dW, db = Linear_ReLU.backward(dout, hidden_caches[i - 1])
...
def rmsprop(w, dw, config=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    cache = config['decay_rate'] * config['cache'] + (1 - config['decay_rate']) * dw.square()
