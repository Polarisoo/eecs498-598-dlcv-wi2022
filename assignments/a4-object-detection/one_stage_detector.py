...
class FCOSPredictionNetwork(nn.Module):
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, feats in feats_per_fpn_level.items():
            ...
            class_logits[level_name] = cls_logits.permute(0, 2, 3, 1).reshape(
                cls_logits.shape[0], -1, cls_logits.shape[1]
            )
            boxreg_deltas[level_name] = box_deltas.permute(0, 2, 3, 1).reshape(
                box_deltas.shape[0], -1, 4
            )
            centerness_logits[level_name] = ctr_logits.permute(0, 2, 3, 1).reshape(
...
def fcos_apply_deltas_to_locations(
    ...
    output_boxes = torch.stack([x - left, y - top, x + right, y + bottom], dim=1)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness[deltas.min(dim=1).values < 0] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        pos_mask = matched_gt_boxes[:, :, 4] >= 0
        gt_classes = matched_gt_boxes[:, :, 4].long().clamp(min=0)
        target_classes = torch.zeros_like(pred_cls_logits)
        pos_batch, pos_location = pos_mask.nonzero(as_tuple=True)
        target_classes[pos_batch, pos_location, gt_classes[pos_mask]] = 1.0
        ...
        target_ctr = fcos_make_centerness_targets(matched_gt_deltas.reshape(-1, 4)).reshape(
            matched_gt_deltas.shape[:2]
        )
        loss_ctr = F.binary_cross_entropy_with_logits(
            pred_ctr_logits.squeeze(-1), target_ctr.clamp(min=0), reduction="none"
        )
